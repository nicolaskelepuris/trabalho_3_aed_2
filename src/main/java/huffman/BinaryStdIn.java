...
public final class BinaryStdIn {
    ...
    // fill buffer
    private static void initialize(FileInputStream stream) {
   ...
   /**
     ...
     */
    public static boolean isEmpty(FileInputStream stream) {
        if (!isInitialized) initialize(stream);
        return buffer == EOF;
    }
   ...
   /**
     ...
     */
    public static boolean readBoolean(FileInputStream stream) {
        if (isEmpty(stream)) throw new NoSuchElementException("Reading from empty input stream");
        n--;
   ...
   /**
     ...
     */
    public static char readChar(FileInputStream stream) {
        if (isEmpty(stream)) throw new NoSuchElementException("Reading from empty input stream");
   ...
   /**
     ...
     */
    public static char readChar(int r, FileInputStream stream) {
        if (r < 1 || r > 16) throw new IllegalArgumentException("Illegal value of r = " + r);

        // optimize r = 8 case
        if (r == 8) return readChar(stream);

        char x = 0;
        for (int i = 0; i < r; i++) {
            x <<= 1;
            boolean bit = readBoolean(stream);
            if (bit) x |= 1;
        }
        return x;
    }
   ...
   /**
     ...
     */
    public static short readShort(FileInputStream stream) {
        short x = 0;
        for (int i = 0; i < 2; i++) {
            char c = readChar(stream);
            x <<= 8;
   ...
   /**
     ...
     */
    public static int readInt(FileInputStream stream) {
        int x = 0;
        for (int i = 0; i < 4; i++) {
            char c = readChar(stream);
            x <<= 8;
   ...
   /**
     ...
     */
    public static int readInt(int r, FileInputStream stream) {
        if (r < 1 || r > 32) throw new IllegalArgumentException("Illegal value of r = " + r);

        // optimize r = 32 case
        if (r == 32) return readInt(stream);
   ...
   /**
     ...
     */
    public static long readLong(FileInputStream stream) {
        long x = 0;
        for (int i = 0; i < 8; i++) {
            char c = readChar(stream);
            x <<= 8;
   ...
   /**
     ...
     * @throws NoSuchElementException if there are fewer than 64 bits available on standard input
     */
    public static double readDouble(FileInputStream stream) {
   ...
   /**
     ...
     * @throws NoSuchElementException if there are fewer than 32 bits available on standard input
     */
    public static float readFloat(FileInputStream stream) {
   ...
   /**
     ...
     * @throws NoSuchElementException if there are fewer than 8 bits available on standard input
     */
    public static byte readByte(FileInputStream stream) {
        char c = readChar(stream);
        return (byte) (c & 0xff);
    }
}